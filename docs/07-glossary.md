...
**Enforcement loop** — The closed cycle that makes organons executable: Define (organon) -> Bind (workflow) -> Execute (tools) -> Verify (automated checks). Without this loop, organons are suggestions.
...
**Triplet** — A complete invariant-to-test binding. Verification checks that invariants declared in ETHOS.md frontmatter have corresponding `@organon-invariant`-annotated tests.
